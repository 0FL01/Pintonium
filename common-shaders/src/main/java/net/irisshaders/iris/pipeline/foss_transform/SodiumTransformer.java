...
public class SodiumTransformer {
    ...
    public static void patchSodiumCore(Transformer translationUnit, SodiumParameters parameters) {
        ...
        injectFragmentCompatibilityDefaults(translationUnit, parameters);
    }
    ...
    private static void injectFragmentCompatibilityDefaults(Transformer translationUnit, SodiumParameters parameters) {
        if (parameters.type != ShaderType.FRAGMENT) {
            return;
        }

        // Some packs guard Distant Horizons helper variables with DH-specific macros, but still read them
        // from the regular terrain/water path. Keep those packs compiling when DH is absent on 1.12.
        if (!translationUnit.hasVariable("old_d")) {
            translationUnit.prependMain("float old_d = gl_FragCoord.z;");
        }

        if (!translationUnit.hasVariable("border_fog_amount")) {
            translationUnit.prependMain("float border_fog_amount = 0.0;");
        }
    }
}
