...
public class IdMap {
	...
	IdMap(Path shaderPath, ShaderPackOptions shaderPackOptions, Iterable<StringPair> environmentDefines) {
		...
		Optional<Properties> entityProperties = loadProperties(shaderPath, "entity.properties", shaderPackOptions, environmentDefines);
		Object2IntMap<NamespacedId> parsedEntityIdMap = entityProperties.map(IdMap::parseEntityIdMap).orElse(Object2IntMaps.emptyMap());
		if (parsedEntityIdMap.isEmpty() && entityProperties.isPresent()) {
			// Some current shaderpacks leave the <=1.12 entity.properties branch empty while
			// their entity shaders still expect the modern named entity ranges.
			parsedEntityIdMap = loadProperties(shaderPath, "entity.properties", shaderPackOptions, forceModernEntityPropertiesVersion(environmentDefines))
				.map(IdMap::parseEntityIdMap).orElse(parsedEntityIdMap);
		}
		entityIdMap = parsedEntityIdMap;
	...
	private static Iterable<StringPair> forceModernEntityPropertiesVersion(Iterable<StringPair> environmentDefines) {
		List<StringPair> modernDefines = new ArrayList<>();
		for (StringPair envDefine : environmentDefines) {
			if (!"MC_VERSION".equals(envDefine.key())) {
				modernDefines.add(envDefine);
			}
		}
		modernDefines.add(new StringPair("MC_VERSION", "11300"));
		return modernDefines;
	}
