...
public class IrisCommon {
	...
	private static boolean loadExternalShaderpack(String name) {
		...
		try {
			...
			OptionValues optionValues = currentPack.getShaderPackOptions().getOptionValues();

			// Store all discovered values so 1.12 users get an editable pack settings file immediately.
			Properties configsToSave = new Properties();
			optionValues.getOptionSet().getBooleanOptions().forEach((k, v) -> configsToSave.setProperty(k, Boolean.toString(optionValues.getBooleanValueOrDefault(k))));
			optionValues.getOptionSet().getStringOptions().forEach((k, v) -> configsToSave.setProperty(k, optionValues.getStringValueOrDefault(k)));
