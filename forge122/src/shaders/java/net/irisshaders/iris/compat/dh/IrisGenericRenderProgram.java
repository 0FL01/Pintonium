...
public class IrisGenericRenderProgram implements IDhApiGenericObjectShaderProgram {
	...
	private IrisGenericRenderProgram(String name, boolean isShadowPass, boolean translucent, BlendModeOverride override, BufferBlendOverride[] bufferBlendOverrides, String vertex, String tessControl, String tessEval, String geometry, String fragment, CustomUniforms customUniforms, CommonIrisRenderingPipeline pipeline) {
		...
		pipeline.addCommonUniforms(uniformBuilder, FogMode.PER_VERTEX);
	...
	public static IrisGenericRenderProgram createProgram(String name, boolean isShadowPass, boolean translucent, ProgramSource source, CustomUniforms uniforms, CommonIrisRenderingPipeline pipeline) {
	...
	public void bind(DhApiRenderParam renderParam) {
		...
		setUniform(modelViewInverseUniform, new Matrix4f(modelView).invert());
		setUniform(projectionUniform, projection);
		setUniform(projectionInverseUniform, new Matrix4f(projection).invert());
		...
		this.setUniform(this.instancedShaderProjectionModelViewMatrixUniform, new Matrix4f(projection).mul(modelView));
	...
	public boolean overrideThisFrame() {
		return IrisCommon.getPipelineManager().getPipelineNullable() instanceof CommonIrisRenderingPipeline;
	}
	...
	private static Matrix4f toJOML(DhApiMat4f mat4f) {
